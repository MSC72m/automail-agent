...
class BrowserInstanceFinder(ABC):
    ...
    @abstractmethod
    def find_browser_instance(self) -> Optional[str]:
        pass
...
class BrowserFinderFactory:
    @staticmethod
    def create_finder(browser_name: str) -> BrowserInstanceFinder:
        platform = sys.platform
        if platform.startswith('linux'):
            return LinuxBrowserInstanceFinder(browser_name)
        elif platform.startswith('win'):
            return WindowsBrowserInstanceFinder(browser_name)
        else:
            raise NotImplementedError(f"Browser finding for platform {platform} is not implemented")


class BrowserLauncher:
    def __init__(self, browser_name: str = "chrome"):
        self.browser_name = browser_name
        self.finder = BrowserFinderFactory.create_finder(browser_name)
        self.browser_path = self.finder.find_browser_instance()
    ...
    async def launch_and_navigate(self, url: str = "https://gmail.com"):
        ...
        async with async_playwright() as p:
            try:
                if self.browser_name.lower() == "chrome":
                    # Instead of using Playwright's launcher, we'll use the system command
                    # to open Chrome with the existing profile
                    import subprocess
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"Warning: Could not check/install Playwright browsers: {e}")
    
    launcher = BrowserLauncher("chrome")
    if launcher.browser_path:
        asyncio.run(launcher.launch_and_navigate())
    else:
        print("Browser not found. Please make sure Chrome is installed.")
